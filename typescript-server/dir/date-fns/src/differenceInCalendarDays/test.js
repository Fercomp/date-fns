...
(0, vitest_1.describe)("differenceInCalendarDays", () => {
    (0, vitest_1.it)("returns the number of calendar days between the given dates", () => {
        const result = (0, index_js_1.differenceInCalendarDays)(new Date(2012, 6 /* Jul */, 2, 18, 0), new Date(2011, 6 /* Jul */, 2, 6, 0));
        (0, vitest_1.expect)(result).toBe(366);
    });
    (0, vitest_1.it)("returns a negative number if the time value of the first date is smaller", () => {
        const result = (0, index_js_1.differenceInCalendarDays)(new Date(2011, 6 /* Jul */, 2, 6, 0), new Date(2012, 6 /* Jul */, 2, 18, 0));
        (0, vitest_1.expect)(result).toBe(-366);
    });
    (0, vitest_1.it)("accepts timestamps", () => {
        const result = (0, index_js_1.differenceInCalendarDays)(new Date(2014, 8 /* Sep */, 5, 18, 0).getTime(), new Date(2014, 8 /* Sep */, 4, 6, 0).getTime());
    ...
    (0, vitest_1.describe)("edge cases", () => {
        (0, vitest_1.it)("the difference is less than a day, but the given dates are in different calendar days", () => {
            const result = (0, index_js_1.differenceInCalendarDays)(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 4, 23, 59));
            (0, vitest_1.expect)(result).toBe(1);
        });
        (0, vitest_1.it)("the same for the swapped dates", () => {
            const result = (0, index_js_1.differenceInCalendarDays)(new Date(2014, 8 /* Sep */, 4, 23, 59), new Date(2014, 8 /* Sep */, 5, 0, 0));
            (0, vitest_1.expect)(result).toBe(-1);
        });
        (0, vitest_1.it)("the time values of the given the given dates are the same", () => {
            const result = (0, index_js_1.differenceInCalendarDays)(new Date(2014, 8 /* Sep */, 6, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
            (0, vitest_1.expect)(result).toBe(1);
        });
        (0, vitest_1.it)("the given the given dates are the same", () => {
            const result = (0, index_js_1.differenceInCalendarDays)(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
        ...
        (0, vitest_1.it)("properly works with negative numbers", () => {
            ...
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(b, a)).toBe(1);
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(a, b)).toBe(-1);
        });
    });
    (0, vitest_1.it)("returns NaN if the first date is `Invalid Date`", () => {
        const result = (0, index_js_1.differenceInCalendarDays)(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
        (0, vitest_1.expect)(isNaN(result)).toBe(true);
    });
    (0, vitest_1.it)("returns NaN if the second date is `Invalid Date`", () => {
        const result = (0, index_js_1.differenceInCalendarDays)(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
        (0, vitest_1.expect)(isNaN(result)).toBe(true);
    });
    (0, vitest_1.it)("returns NaN if the both dates are `Invalid Date`", () => {
        const result = (0, index_js_1.differenceInCalendarDays)(new Date(NaN), new Date(NaN));
        (0, vitest_1.expect)(isNaN(result)).toBe(true);
    });
    ...
    dstOnly(`works across DST start & end in local timezone: ${tz || "(unknown)"}`, () => {
        ...
        // anchor to one hour before the boundary
        {
            ...
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, b)).toBe(1); // normal 24-hour day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(b, a)).toBe(1); // 23 hours -> 1 day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, a)).toBe(2); // 47 hours -> 2 days
        }
        // anchor exactly at the boundary
        {
            ...
            (0, vitest_1.expect)(sameTime(b, c)).toBe(true);
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, b)).toBe(1); // normal 24-hour day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(b, a)).toBe(1); // normal 24-hour day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, a)).toBe(2); // 2 normal 24-hour days
        }
        // TEST DST END (FALL)
        ...
        // until 25 hours have elapsed.
        {
            ...
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, b)).toBe(1); // normal 24-hour day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(b, a)).toBe(1); // 24.75 hours but 1 calendar days
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, a)).toBe(2); // 49.75 hours but 2 calendar days
        }
        // anchor to one hour before the boundary
        {
            ...
            (0, vitest_1.expect)(sameTime(b, c)).toBe(true);
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, b)).toBe(1); // normal 24-hour day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(b, a)).toBe(1); // 25 hours -> 1 day
            (0, vitest_1.expect)((0, index_js_1.differenceInCalendarDays)(c, a)).toBe(2); // 49 hours -> 2 days
        }
