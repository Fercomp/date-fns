"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const vitest_1 = require("vitest");
const index_js_1 = require("./index.js");
(0, vitest_1.describe)("eachMinuteOfInterval", () => {
    (0, vitest_1.it)("should return an array of Date objects containing a Date for each minute between the interval", () => {
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start: new Date(2020, 10, 14, 13, 0),
            end: new Date(2020, 10, 14, 13, 5),
        });
        (0, vitest_1.expect)(result).toEqual([
            new Date(2020, 10, 14, 13, 0),
            new Date(2020, 10, 14, 13, 1),
            new Date(2020, 10, 14, 13, 2),
            new Date(2020, 10, 14, 13, 3),
            new Date(2020, 10, 14, 13, 4),
            new Date(2020, 10, 14, 13, 5),
        ]);
    });
    (0, vitest_1.it)("should handle all the minutes that are not in the begining", () => {
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start: new Date(2020, 10, 14, 13, 0, 33),
            end: new Date(2020, 10, 14, 13, 2),
        });
        (0, vitest_1.expect)(result[0]).toEqual(new Date(2020, 10, 14, 13));
        (0, vitest_1.expect)(result[2]).toEqual(new Date(2020, 10, 14, 13, 2));
    });
    (0, vitest_1.it)("should accept timestamps", () => {
        const start = new Date(2020, 10, 14, 13, 0).getTime();
        const end = new Date(2020, 10, 14, 13, 2).getTime();
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start,
            end,
        });
        (0, vitest_1.expect)(result).toEqual([
            new Date(2020, 10, 14, 13, 0),
            new Date(2020, 10, 14, 13, 1),
            new Date(2020, 10, 14, 13, 2),
        ]);
    });
    (0, vitest_1.it)("treats intervals shorter than a minute as valid", () => {
        const block = index_js_1.eachMinuteOfInterval.bind(null, {
            start: new Date(2014, 10, 14, 10, 1, 0),
            end: new Date(2014, 10, 14, 10, 1, 1),
        });
        (0, vitest_1.expect)(block).not.toThrow();
    });
    (0, vitest_1.it)("returns reversed array if the start date is after the end date", () => {
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start: new Date(2020, 10, 14, 13, 5),
            end: new Date(2020, 10, 14, 13, 0),
        });
        (0, vitest_1.expect)(result).toEqual([
            new Date(2020, 10, 14, 13, 5),
            new Date(2020, 10, 14, 13, 4),
            new Date(2020, 10, 14, 13, 3),
            new Date(2020, 10, 14, 13, 2),
            new Date(2020, 10, 14, 13, 1),
            new Date(2020, 10, 14, 13, 0),
        ]);
    });
    (0, vitest_1.it)("returns an empty array if the start date is `Invalid Date`", () => {
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start: new Date(NaN),
            end: new Date(2014, 9 /* Oct */, 6),
        });
        (0, vitest_1.expect)(result).toEqual([]);
    });
    (0, vitest_1.it)("returns an empty array if the end date is `Invalid Date`", () => {
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start: new Date(2014, 9 /* Oct */, 12),
            end: new Date(NaN),
        });
        (0, vitest_1.expect)(result).toEqual([]);
    });
    (0, vitest_1.it)("returns an empty array if both of the properties are `Invalid Date`", () => {
        const result = (0, index_js_1.eachMinuteOfInterval)({
            start: new Date(NaN),
            end: new Date(NaN),
        });
        (0, vitest_1.expect)(result).toEqual([]);
    });
    (0, vitest_1.describe)("options.step", () => {
        const interval = {
            start: new Date(2020, 9, 14, 13, 1),
            end: new Date(2020, 9, 14, 13, 7),
        };
        (0, vitest_1.it)("returns an array with starts of hours from the hour of the start date to the hour of the end date with the given step", () => {
            const result = (0, index_js_1.eachMinuteOfInterval)(interval, { step: 3 });
            (0, vitest_1.expect)(result).toEqual([
                new Date(2020, 9, 14, 13, 1),
                new Date(2020, 9, 14, 13, 4),
                new Date(2020, 9, 14, 13, 7),
            ]);
        });
        (0, vitest_1.it)("returns reversed array if `options.step` is negative", () => {
            const result = (0, index_js_1.eachMinuteOfInterval)({
                start: new Date(2020, 10, 14, 13, 0),
                end: new Date(2020, 10, 14, 13, 5),
            }, { step: -1 });
            (0, vitest_1.expect)(result).toEqual([
                new Date(2020, 10, 14, 13, 5),
                new Date(2020, 10, 14, 13, 4),
                new Date(2020, 10, 14, 13, 3),
                new Date(2020, 10, 14, 13, 2),
                new Date(2020, 10, 14, 13, 1),
                new Date(2020, 10, 14, 13, 0),
            ]);
        });
        (0, vitest_1.it)("reverses array twice if `options.step` is negative and the interval is negative too", () => {
            const result = (0, index_js_1.eachMinuteOfInterval)({
                start: new Date(2020, 10, 14, 13, 5),
                end: new Date(2020, 10, 14, 13, 0),
            }, { step: -1 });
            (0, vitest_1.expect)(result).toEqual([
                new Date(2020, 10, 14, 13, 0),
                new Date(2020, 10, 14, 13, 1),
                new Date(2020, 10, 14, 13, 2),
                new Date(2020, 10, 14, 13, 3),
                new Date(2020, 10, 14, 13, 4),
                new Date(2020, 10, 14, 13, 5),
            ]);
        });
        (0, vitest_1.it)("returns empty array if `options.step` is less than 1", () => {
            const result = (0, index_js_1.eachMinuteOfInterval)(interval, { step: 0 });
            (0, vitest_1.expect)(result).toEqual([]);
        });
        (0, vitest_1.it)("returns empty array if `options.step` is NaN", () => {
            const result = (0, index_js_1.eachMinuteOfInterval)(interval, { step: NaN });
            (0, vitest_1.expect)(result).toEqual([]);
        });
    });
});
